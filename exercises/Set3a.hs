...
firstHalf x = fst (splitAt (ceiling (fromIntegral (length x) / 2.0)) x)
...
capitalize x = unwords (map (\y -> (toUpper (head y)) : (tail y)) (words x))
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (max >=) (zipWith (^) (take (max + 1) [k, k ..]) [0 .. max])
...
whileRight f x = either (\l -> l) (\r -> whileRight f r) (f x)
...
joinToLength n l = filter (\x -> length x == n) (concat (map (\x -> map (++ x) l) l))
...
sumRights l = foldr (+) 0 (map (mapValid) l)
  where mapValid (Right num) = num
        mapValid (Left str) = 0
...
multiCompose [] o = o
multiCompose fs o = multiCompose (init fs) ((last fs) o)
...
multiApp f gs x = f (map (\o -> (o x)) gs)
...
interpreter [] = []
interpreter commands = interpreterRec (head commands) 0 0 (tail commands) []
  where interpreterRec c x y [] res = snd (applyCommand c x y res)
        interpreterRec c x y commands res = interpreterRec (head commands) (fst (fst (applyCommand c x y res))) (snd (fst (applyCommand c x y res))) (tail commands) (snd (applyCommand c x y res))
        -- apply commands
        applyCommand "up" x y res = ((x, y + 1), res)
        applyCommand "down" x y res = ((x, y - 1), res)
        applyCommand "left" x y res = ((x - 1, y), res)
        applyCommand "right" x y res = ((x + 1, y), res)
        applyCommand "printX" x y res = ((x, y), (res ++ [show x]))
        applyCommand "printY" x y res = ((x, y), (res ++ [show y]))
        applyCommand c x y res = ((x, y), res)
