...
allValues condition (Node a l r) = if condition a then allValues condition l && allValues condition r else False
...
walk (StepL:steps) (Node a l r) = walk steps l
walk (StepR:steps) (Node a l r) = walk steps r
...
search val n = if fst (doSearch val n) then (Just (snd (doSearch val n))) else Nothing
    where doSearch val n = searchRec val n []
          searchRec val Empty steps = (False, [])
          searchRec val (Node a l r) steps =
                          if val == a
                          then (True, reverse steps)
                          else if not (fst (searchLeft val l steps))
                               then searchRight val r steps
                               else searchLeft val l steps
          searchLeft val l steps = searchRec val l (StepL:steps)
          searchRight val r steps = searchRec val r (StepR:steps)
