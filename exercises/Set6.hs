...
instance Ord Number where
  ...
  compare (Infinite) f = GT
  compare f (Infinite) = LT
  (<=) x y = compare x y /= GT
  min x y
    | x <= y = x
    | otherwise = y
...
instance Eq RationalNumber where
  (RationalNumber i11 i12) == (RationalNumber i21 i22) = i11 * i22 == i12 * i21
...
simplify (RationalNumber numerator denominator) = let g = gcd numerator denominator
                                                  in (RationalNumber (numerator `div` g) (denominator `div` g))
...
instance Num RationalNumber where
  (RationalNumber i11 i12) + (RationalNumber i21 i22) = simplify (RationalNumber ((i11 * i22) + (i21 * i12)) (i12 * i22))
  (RationalNumber i11 i12) * (RationalNumber i21 i22) = simplify (RationalNumber (i11 * i21) (i12 * i22))
  abs (RationalNumber i1 i2)  = RationalNumber (abs i1) (abs i2)
  signum (RationalNumber i1 i2)
    | i1 == 0 = RationalNumber 0 1
    | (i1 > 0 && i2 > 0) || (i1 < 0 && i2 < 0) = RationalNumber 1 1
    | otherwise = RationalNumber (-1) 1
  fromInteger x = RationalNumber x 1
  negate (RationalNumber i1 i2) = RationalNumber (-i1) i2
