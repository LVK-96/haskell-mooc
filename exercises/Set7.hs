...
member a (Set l) = elem a l

-- add a member to a set
add :: (Eq a, Ord a) => a -> Set a -> Set a
add a (Set l) = if member a (Set l) then (Set l) else Set (sort (a:l))
...
data State = Start | EggsAdded | FlourAdded | SugarAdded | SugarAndFlourAdded | Mixed | Error | Finished
  deriving (Eq,Show)
...
average l = (foldr (+) 0 l) / fromIntegral (length l)
...
reverseNonEmpty (x:|l) = let reversed = reverse (x:l)
...
instance Ord a => Semigroup (Set a) where
    (<>) (Set l1) (Set l2) = Set (map head (group $ sort (l1 ++ l2)))
...
passwordAllowed pass (ContainsSome req) = elem True (map (\x -> elem x pass) req)
passwordAllowed pass (DoesNotContain req) = not (passwordAllowed pass (ContainsSome req))
