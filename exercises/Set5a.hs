...
totalPrice (MkShoppingEntry _ price amount) = (fromIntegral amount) * price
...
tick :: UpDown -> UpDown
tick (MkUp v) = MkUp (v + 1)
tick (MkDown v) = MkDown (v - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (MkUp v) = MkDown v
toggle (MkDown v) = MkUp v
...
rgb (Mix c1 c2) = map (\x -> ((fst x) + (snd x)) / 2) (zip (rgb c1) (rgb c2))
...
fromList [] = Empty
fromList (e:l) = Pair (fst e) (snd e) (fromList l)
...
toNat :: Int -> Maybe Nat
toNat n = if n < 0 then Nothing else Just (toNatRec n)
    where toNatRec 0 = Zero
          toNatRec n = PlusOne (toNatRec (n - 1))
...
fromBin :: Bin -> Int
fromBin b = fromBinRec b 0 0
    where fromBinRec End n _ = n
          fromBinRec (O b) n l = fromBinRec b n (l + 1)
          fromBinRec (I b) n l = fromBinRec b (n + 2^l) (l + 1)

toBin :: Int -> Bin
toBin n = toBinRec n (O End)
    where toBinRec 0 b = b
          toBinRec n b = toBinRec (n - 1) (inc b)
